...
public class A_DFS {
    ...
    public static Integer solution(String input) throws IOException {
        ...
        // 그래프의 간선 연결 정보를 이차원 배열로 포현
        map = new int[node + 1][node + 1];  // 0번 인덱스를 제외하고 사용하기 위함

        // 방문 배열 생성(지나간 노드를 다시 방문하지 않기 위함)
        visit = new boolean[node + 1];

        StringTokenizer st; // " " 자르는데 사용 , split와 같지만 빠름
        // map에 그래프 내부 정보 작성
    ...
    private static void dfsStack(int start){
        ...
        while (!stack.isEmpty()){       // start 가 1일때 2와 3이 push 됨 그럼 while문을 통해 2가 pop 되고 반복
